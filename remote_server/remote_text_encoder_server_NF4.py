...
from transformers import BitsAndBytesConfig  # 新增
...
def load_model_and_processor():
    ...
    if USE_4BIT and DEVICE.type == "cuda":
        print("[RemoteTextEncoder] Using 4bit quantization with bitsandbytes ...")
        ...
        # 4bit 情况下不要再手动指定 torch_dtype / device_map={"": DEVICE.type}
        # 统一交给 transformers + accelerate + bitsandbytes 管
        ...
        # 原来的 FP16/BF16 路径（非 CUDA 或不想量化时走这里）
        model = Qwen2_5_VLForConditionalGeneration.from_pretrained(
            MODEL_ID,
            torch_dtype=DTYPE,
